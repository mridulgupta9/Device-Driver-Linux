...
MODULE_AUTHOR("Batch 10");
MODULE_DESCRIPTION("Joystick 2 Mouse interfaces");
MODULE_SUPPORTED_DEVICE("js2ms");
MODULE_LICENSE("GPL");
...
{
	// printk(KERN_ALERT "%d %d %d ", type, code, value);
	struct joydev *joydev = handle->private;
	
	struct joydev_client *client;
	struct js_event event;
	printk(KERN_ALERT "A0 %d A1 %d A2 %d A3 %d A4 %d A5 %d B0 %d B1 %d B2 %d B3 %d B4 %d B5 %d B6 %d B7 %d B8 %d B9 %d B10 %d B11 %d\n",joydev->abs[0],joydev->abs[1],joydev->abs[2],joydev->abs[3],joydev->abs[4],joydev->abs[5],joydev->key[0],joydev->key[1],joydev->key[2],joydev->key[3],joydev->key[4],joydev->key[5],joydev->key[6],joydev->key[7],joydev->key[8],joydev->key[9],joydev->key[10],joydev->key[11]);
	struct input_dev *dev = mouse->idev;
	
	input_report_rel(dev, REL_X, joydev->abs[0]);
	

	
	input_report_rel(dev, REL_Y, joydev->abs[1]);
	
	
	input_report_key(dev, BTN_RIGHT, joydev->key[1]);
	
	

	input_report_key(dev, BTN_LEFT, joydev->key[3]);

	//scroll

	input_report_rel(dev, REL_WHEEL, joydev->abs[3]/2);
	...
	case EV_KEY:
		...
		event.value = value;
		joydev->key[event.number] = event.value;
		break;
...
static int joydev_generate_startup_event(struct joydev_client *client, struct input_dev *input, struct js_event *event)
{
	struct joydev *joydev = client->joydev;
	int have_event;
...
static int joydev_fetch_next_event(struct joydev_client *client,    struct js_event *event)
{
	int have_event;
 ...
 */
static ssize_t joydev_0x_read(struct joydev_client *client,  struct input_dev *input,  char __user *buf)
{
	struct joydev *joydev = client->joydev;
...
static int joydev_handle_JSIOCSAXMAP(struct joydev *joydev, void __user *argp, size_t len)
{
	__u8 *abspam;
	int i;
	int retval = 0;
...
static int joydev_handle_JSIOCSBTNMAP(struct joydev *joydev,       void __user *argp, size_t len)
{
	__u16 *keypam;
	int i;
	int retval = 0;
...
{
	printk(KERN_DEBUG "joydev_ioctl_common");
	struct input_dev *dev = joydev->handle.dev;
	size_t len;
	int i;
	const char *name;
	...
	case JS_SET_CAL:
		return copy_from_user(&joydev->glue.JS_CORR, argp,sizeof(joydev->glue.JS_CORR)) ? -EFAULT : 0;

	case JS_GET_CAL:
		return copy_to_user(argp, &joydev->glue.JS_CORR,sizeof(joydev->glue.JS_CORR)) ? -EFAULT : 0;
...
{
	printk(KERN_DEBUG "joydev_compat_ioctl");
	struct joydev_client *client = file->private_data;
	struct joydev *joydev = client->joydev;
	...
	case JS_SET_ALL:
		retval = copy_from_user(&ds32, argp,sizeof(ds32)) ? -EFAULT : 0;
		if (retval == 0) {
...
{
	printk(KERN_DEBUG "joydev_ioctl");
	struct joydev_client *client = file->private_data;
	struct joydev *joydev = client->joydev;
	void __user *argp = (void __user *)arg;
	int retval;
	...
	case JS_SET_TIMELIMIT:
		retval = get_user(joydev->glue.JS_TIMELIMIT,(long __user *) arg);
		break;

	case JS_GET_TIMELIMIT:
		retval = put_user(joydev->glue.JS_TIMELIMIT,(long __user *) arg);
		break;

	case JS_SET_ALL:
		retval = copy_from_user(&joydev->glue, argp,sizeof(joydev->glue)) ? -EFAULT : 0;
		break;

	case JS_GET_ALL:
		retval = copy_to_user(argp, &joydev->glue, sizeof(joydev->glue)) ? -EFAULT : 0;
		break;
...
{
	printk(KERN_DEBUG "joydev_dev_is_absolute_mouse");
...
static int joydev_connect(struct input_handler *handler, struct input_dev *dev,  const struct input_device_id *id)
{
	struct joydev *joydev;
	...
	dev_set_name(&joydev->dev, "js2ms%d", dev_no);
...
static const int register_mouse(void)
{
	int retval;	
	// Major = register_chrdev(0, "mouse1", &mousek_fops);
	// if (Major < 0) {
